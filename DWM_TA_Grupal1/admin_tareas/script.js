...
saveTaskBtn.addEventListener('click', () => {
    ...
    // Refrescar todas las tareas después de guardar o editar
    fetchTasksFromServer();
});

// Función para traer las tareas del servidor y actualizar la UI
function fetchTasksFromServer() {
    fetch('http://localhost:3000/api/tasks')
        .then(response => response.json())
        .then(data => {
            // Limpiar las columnas antes de agregar las tareas actualizadas
            clearTaskColumns();

            // Crear cada tarea y agregarla a la UI
            data.forEach(task => {
                createTask(task);
            });
        })
...
function clearTaskColumns() {
    document.querySelectorAll('.box').forEach(box => {
        box.innerHTML = '<h2 class="subtitle">' + box.querySelector('h2.subtitle').textContent + '</h2>';
    });
}
...
function createTaskOnServer(taskData) {
    ...
    .then(response => response.json())
    .then(createdTask => {
        fetchTasksFromServer(); // Refrescar las tareas después de crear una nueva
    })
    .catch(error => {
...
// Actualización de la tarea en el servidor
function updateTaskOnServer(taskCard, taskData) {
    ...
    .then(response => response.json())
    .then(updatedTask => {
        fetchTasksFromServer(); // Refrescar las tareas después de actualizar una tarea existente
    })
    .catch(error => {
...
// Inicialización: cargar tareas cuando la página se carga
document.addEventListener('DOMContentLoaded', fetchTasksFromServer);
